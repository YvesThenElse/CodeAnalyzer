...
var objectIs$1 = "function" === typeof Object.is ? Object.is : is$1, useState = React$2.useState, useEffect$1 = React$2.useEffect, useLayoutEffect = React$2.useLayoutEffect, useDebugValue$2 = React$2.useDebugValue;
...
var objectIs = "function" === typeof Object.is ? Object.is : is, useSyncExternalStore = shim.useSyncExternalStore, useRef = React$1.useRef, useEffect = React$1.useEffect, useMemo = React$1.useMemo, useDebugValue$1 = React$1.useDebugValue;
...
function useStore(api, selector2 = identity) {
  const slice = React$3.useSyncExternalStore(
    api.subscribe,
    React$3.useCallback(() => selector2(api.getState()), [api, selector2]),
    React$3.useCallback(() => selector2(api.getInitialState()), [api, selector2])
  );
...
var C4Level = /* @__PURE__ */ ((C4Level2) => {
  C4Level2[C4Level2["SYSTEM_CONTEXT"] = 1] = "SYSTEM_CONTEXT";
  C4Level2[C4Level2["CONTAINER"] = 2] = "CONTAINER";
  C4Level2[C4Level2["COMPONENT"] = 3] = "COMPONENT";
  C4Level2[C4Level2["CODE"] = 4] = "CODE";
  return C4Level2;
})(C4Level || {});
var C4NodeType = /* @__PURE__ */ ((C4NodeType2) => {
  C4NodeType2["PERSON"] = "person";
  C4NodeType2["SYSTEM"] = "system";
  C4NodeType2["EXTERNAL_SYSTEM"] = "external_system";
  C4NodeType2["CLOUD_SERVICE"] = "cloud_service";
  C4NodeType2["CONTAINER_FRONTEND"] = "container_frontend";
  C4NodeType2["CONTAINER_BACKEND"] = "container_backend";
  C4NodeType2["CONTAINER_DATABASE"] = "container_database";
  C4NodeType2["COMPONENT"] = "component";
  C4NodeType2["CODE_FUNCTION"] = "code_function";
  C4NodeType2["CODE_COMPONENT"] = "code_component";
  C4NodeType2["CODE_HOOK"] = "code_hook";
  return C4NodeType2;
})(C4NodeType || {});
...
function findElementById(project, id2) {
  for (const level of Object.values(project.levels)) {
    const found = level.find((el2) => el2.id === id2);
    if (found) return found;
  }
  return void 0;
}
function getReactFlowNodeType(c4Type) {
  switch (c4Type) {
...
const useC4Store = create((set2, get2) => ({
  // Initial state
  project: null,
  isLoading: false,
  error: null,
  ...
  // Actions
  setProject: (project) => set2({ project, error: null }),
  setLoading: (isLoading) => set2({ isLoading }),
  setError: (error) => set2({ error, isLoading: false }),
  setProgress: (progress) => set2({ progress }),
  setCurrentLevel: (level) => set2({ currentLevel: level }),
  setCurrentElementId: (id2) => set2({ currentElementId: id2 }),
  addToBreadcrumb: (elementId) => {
    const { project, breadcrumb, currentLevel } = get2();
    if (!project) return;
  ...
  navigateToBreadcrumb: (elementId) => {
    const { breadcrumb } = get2();
    const index2 = breadcrumb.findIndex((b) => b.id === elementId);
    if (index2 === -1) return;
    const item = breadcrumb[index2];
    set2({
      currentLevel: item.level,
      currentElementId: item.id === "root" ? null : item.id,
      breadcrumb: breadcrumb.slice(0, index2 + 1)
    });
  },
  reset: () => set2({
    project: null,
  ...
  getVisibleNodesAndEdges: () => {
    const { project, currentLevel, currentElementId } = get2();
    if (!project) {
      return { nodes: [], edges: [] };
    }
    const levelElements = project.levels[currentLevel] || [];
    const elements = currentElementId ? levelElements.filter((el2) => el2.parentId === currentElementId) : levelElements;
    const nodes = elements.map((el2) => ({
      id: el2.id,
      type: getReactFlowNodeType(el2.type),
      position: { x: 0, y: 0 },
      // Will be calculated by layout
      data: {
        element: el2,
        isClickable: currentLevel < C4Level.CODE,
        hasChildren: (el2.children?.length ?? 0) > 0
      }
    }));
    const elementIds = new Set(elements.map((e) => e.id));
    const edges = project.relations.filter((r2) => elementIds.has(r2.sourceId) && elementIds.has(r2.targetId)).map((r2) => ({
      id: r2.id,
      source: r2.sourceId,
      target: r2.targetId,
      type: "c4Edge",
...
function px(node, styleProperty) {
  const win2 = node.ownerDocument.defaultView || window;
  const val = win2.getComputedStyle(node).getPropertyValue(styleProperty);
...
function getImageSize(targetNode, options = {}) {
  const width2 = options.width || getNodeWidth(targetNode);
...
async function nodeToDataURL(node, width2, height) {
...
async function toSvg(node, options = {}) {
  const { width: width2, height } = getImageSize(node, options);
  const clonedNode = await cloneNode(node, options, true);
  await embedWebFonts(clonedNode, options);
  await embedImages(clonedNode, options);
  applyStyle(clonedNode, options);
  const datauri = await nodeToDataURL(clonedNode, width2, height);
  return datauri;
}
async function toCanvas(node, options = {}) {
  ...
  const canvasWidth = options.canvasWidth || width2;
...
function generateFileName(ext) {
  ...
  return `c4-diagram-${date2}.${ext}`;
}
function useExport() {
  const { getNodes } = useReactFlow();
  const { project, currentLevel, breadcrumb } = useC4Store();
  const exportPng = reactExports.useCallback(async () => {
  ...
  const exportJson = reactExports.useCallback(async () => {
    if (!project) return;
    const exportData = {
      name: project.name,
...
function ExportMenu({ disabled }) {
  ...
  const handleExportPng = reactExports.useCallback(async () => {
    setIsOpen(false);
    await exportPng();
  }, [exportPng]);
  const handleExportJson = reactExports.useCallback(async () => {
    setIsOpen(false);
    await exportJson();
  }, [exportJson]);
  return /* @__PURE__ */ React.createElement("div", { className: `export-menu ${isOpen ? "export-menu--open" : ""}`, ref: menuRef }, /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    "Export",
    /* @__PURE__ */ React.createElement("span", { className: "export-menu__arrow" }, "▼")
  ), isOpen && /* @__PURE__ */ React.createElement("ul", { className: "export-menu__dropdown" }, /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("button", { onClick: handleExportPng }, /* @__PURE__ */ React.createElement("span", { className: "icon" }, "🖼️"), "Export PNG")), /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("button", { onClick: handleExportJson }, /* @__PURE__ */ React.createElement("span", { className: "icon" }, "📄"), "Export JSON"))));
}
function Header() {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "header" }, /* @__PURE__ */ React.createElement("div", { className: "header__logo" }, "ReactAnalyzer"), /* @__PURE__ */ React.createElement("div", { className: "header__actions" }, /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    "← Back"
  )), /* @__PURE__ */ React.createElement("div", { className: "header__spacer" }), /* @__PURE__ */ React.createElement(ExportMenu, { disabled: !project || isLoading }));
}
const levelLabels = {
...
function Breadcrumb() {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "breadcrumb" }, breadcrumb.map((item, index2) => {
    const isLast = index2 === breadcrumb.length - 1;
    return /* @__PURE__ */ React.createElement("div", { key: item.id, style: { display: "flex", alignItems: "center", gap: "4px" } }, index2 > 0 && /* @__PURE__ */ React.createElement("span", { className: "breadcrumb__separator" }, ">"), /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      item.name,
      /* @__PURE__ */ React.createElement("span", { className: "breadcrumb__level" }, "(", levelLabels[item.level], ")")
    ));
  }));
}
const nodeColors = {
...
function BaseC4Node({ data, selected: selected2 }) {
  const { element, hasChildren } = data;
  const colors = getNodeColors(element.type);
  return /* @__PURE__ */ React.createElement(
    "div",
    {
      className: `c4-node c4-node--${element.type} ${selected2 ? "selected" : ""}`,
      style: {
    ...
    /* @__PURE__ */ React.createElement(Handle, { type: "target", position: Position.Top }),
    /* @__PURE__ */ React.createElement("div", { className: "c4-node__content" }, /* @__PURE__ */ React.createElement("h3", { className: "c4-node__title" }, element.name), /* @__PURE__ */ React.createElement("p", { className: "c4-node__description" }, element.description), hasChildren && /* @__PURE__ */ React.createElement("span", { className: "c4-node__indicator" }, "Double-clic pour explorer")),
    /* @__PURE__ */ React.createElement(Handle, { type: "source", position: Position.Bottom })
  );
}
function C4SystemNode({ data, selected: selected2 }) {
  return /* @__PURE__ */ React.createElement(BaseC4Node, { data, selected: selected2 });
}
function C4ContainerNode({ data, selected: selected2 }) {
  return /* @__PURE__ */ React.createElement(BaseC4Node, { data, selected: selected2 });
}
function C4ComponentNode({ data, selected: selected2 }) {
  return /* @__PURE__ */ React.createElement(BaseC4Node, { data, selected: selected2 });
}
function C4CodeNode({ data, selected: selected2 }) {
  return /* @__PURE__ */ React.createElement(BaseC4Node, { data, selected: selected2 });
}
function C4Edge({
  id: id2,
  ...
  ), label && /* @__PURE__ */ React.createElement(EdgeLabelRenderer, null, /* @__PURE__ */ React.createElement(
    "div",
    {
      style: {
        position: "absolute",
...
function DiagramView() {
  ...
  const onNodeDoubleClick = reactExports.useCallback(
    (_event, node) => {
      if (canDrillDown && node.data.hasChildren) {
        drillDown(node.data.element.id);
      }
    },
    [drillDown, canDrillDown]
  );
  if (!project) {
    return /* @__PURE__ */ React.createElement("div", { className: "empty-state" }, /* @__PURE__ */ React.createElement("div", { className: "empty-state__icon" }, "📊"), /* @__PURE__ */ React.createElement("h2", { className: "empty-state__title" }, "Aucun projet analysé"), /* @__PURE__ */ React.createElement("p", { className: "empty-state__description" }, "Sélectionnez un répertoire contenant un projet React pour générer un diagramme C4 interactif."));
  }
  if (nodes.length === 0) {
    return /* @__PURE__ */ React.createElement("div", { className: "empty-state" }, /* @__PURE__ */ React.createElement("div", { className: "empty-state__icon" }, "📁"), /* @__PURE__ */ React.createElement("h2", { className: "empty-state__title" }, "Aucun élément à ce niveau"), /* @__PURE__ */ React.createElement("p", { className: "empty-state__description" }, "Ce niveau ne contient pas d'éléments. Utilisez le breadcrumb pour naviguer vers un autre niveau."));
  }
  return /* @__PURE__ */ React.createElement("div", { className: "diagram-view" }, /* @__PURE__ */ React.createElement(
    index,
    {
      nodes,
      edges,
    ...
    /* @__PURE__ */ React.createElement(Background, { color: "#E0E0E0", gap: 20 }),
...
function LoadingOverlay() {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "loading-overlay" }, /* @__PURE__ */ React.createElement("div", { className: "loading-overlay__content" }, /* @__PURE__ */ React.createElement("div", { className: "loading-overlay__spinner" }), /* @__PURE__ */ React.createElement("div", { className: "loading-overlay__phase" }, progress ? phaseLabels[progress.phase] : "Chargement..."), progress?.currentFile && /* @__PURE__ */ React.createElement("div", { className: "loading-overlay__file", title: progress.currentFile }, progress.currentFile.split(/[/\\]/).pop()), progress && /* @__PURE__ */ React.createElement("div", { className: "loading-overlay__progress" }, /* @__PURE__ */ React.createElement(
    "div",
    {
      className: "loading-overlay__progress-bar",
      style: { width: `${percentage}%` }
    }
  )), progress && /* @__PURE__ */ React.createElement("div", { style: { fontSize: "12px", color: "#666" } }, progress.current, " / ", progress.total, " fichiers"), /* @__PURE__ */ React.createElement("button", { className: "btn btn--danger loading-overlay__cancel", onClick: handleCancel }, "Annuler")));
}
function DetectionPanel() {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: `detection-panel ${isOpen ? "detection-panel--open" : ""}` }, /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    /* @__PURE__ */ React.createElement("span", { className: "detection-panel__icon" }, "🔍"),
    /* @__PURE__ */ React.createElement("span", { className: "detection-panel__count" }, project.externalSystems.length)
  ), isOpen && /* @__PURE__ */ React.createElement("div", { className: "detection-panel__content" }, /* @__PURE__ */ React.createElement("div", { className: "detection-panel__header" }, /* @__PURE__ */ React.createElement("h3", null, "Systèmes externes détectés"), /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    "×"
  )), /* @__PURE__ */ React.createElement("div", { className: "detection-panel__list" }, project.externalSystems.map((system, index2) => /* @__PURE__ */ React.createElement(SystemItem, { key: index2, system })))));
}
function SystemItem({ system }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "system-item" }, /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    /* @__PURE__ */ React.createElement("span", { className: "system-item__name" }, system.name),
    /* @__PURE__ */ React.createElement("span", { className: "system-item__detections" }, system.detections.length, " détection", system.detections.length > 1 ? "s" : ""),
    /* @__PURE__ */ React.createElement("span", { className: `system-item__arrow ${expanded ? "expanded" : ""}` }, "▼")
  ), expanded && /* @__PURE__ */ React.createElement("ul", { className: "system-item__details" }, system.detections.map((detection, index2) => /* @__PURE__ */ React.createElement("li", { key: index2, className: "detection-detail" }, /* @__PURE__ */ React.createElement("code", { className: "detection-detail__source" }, detection.source), /* @__PURE__ */ React.createElement("span", { className: "detection-detail__location" }, detection.file.split(/[/\\]/).pop(), ":", detection.line)))));
}
function App() {
  const isLoading = useC4Store((state) => state.isLoading);
  return /* @__PURE__ */ React.createElement(ReactFlowProvider, null, /* @__PURE__ */ React.createElement("div", { className: "app" }, /* @__PURE__ */ React.createElement("header", { className: "app__header" }, /* @__PURE__ */ React.createElement(Header, null)), /* @__PURE__ */ React.createElement("nav", { className: "app__breadcrumb" }, /* @__PURE__ */ React.createElement(Breadcrumb, null)), /* @__PURE__ */ React.createElement("main", { className: "app__content" }, /* @__PURE__ */ React.createElement(DiagramView, null)), /* @__PURE__ */ React.createElement(DetectionPanel, null), isLoading && /* @__PURE__ */ React.createElement(LoadingOverlay, null)));
}
client.createRoot(document.getElementById("root")).render(
  /* @__PURE__ */ React$3.createElement(React$3.StrictMode, null, /* @__PURE__ */ React$3.createElement(App, null))
);
